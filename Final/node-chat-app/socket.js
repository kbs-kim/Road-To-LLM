//메시지 서버 소켓파일입니다.
//해당 socket.js모듈이 메시징 서버 역할을 제공합니다.
...
module.exports = (server) => {
    ...
    //서버 소켓의 입출력(In/Out)메시지 처리 객체 io생성
    //input 메시지는 웹브라우저에서 들어오는 메시지
    //output 메시지는 웹브라우저로 전송하는 메시지 
  const io = SocketIO(server, { path: "/socket.io" });

  //자바스크립트에서 on(이벤트핸들러(처리기))
  //io객체에 connection이벤트가 발생하면 콜백함수를 실행하라.
  //connection이벤트는 웹브라우저와 서버소켓과 연결이 완료되면 발생합니다.
  io.on("connection", (socket) => {
    ...
    socket.on("broadcast",function(msg){
        //현재 메시지서버에 연결된 모든 사용자들(웹브라우저/프론트엔드웹페이지)에게
        //메시지를 전송하는데 클라이언트 메시지 수신 이벤트 receiveAll로 msg데이터를 전송한다.
        //io.emit()메소드는 서버소켓(io) 연결된 모든 사용자에게 메시지를 보낼때 사용한다.
