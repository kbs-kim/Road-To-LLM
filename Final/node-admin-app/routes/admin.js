...
router.get("/list", async (req, res, next) => {
    ...
    //모든 데이터 목록에서 내가 원하는 컬럼목록만 조회하기^^^
    // //SELECT admin_member_id,admin_name,email,company_code,dept_name,used_yn_code,reg_date FROM admin;
    // const admins = await db.Admin.findAll({
    //     attributes:['admin_member_id','admin_name','email','company_code','dept_name','used_yn_code','reg_date'],
    //     where:{used_yn_code:1},
    //     order:[['admin_member_id','DESC']]
    // });

    //예시코드: 순수 SQL구문을 DB서버에 전달해서 동일한 결과값 받아오기
    let query = `SELECT 
	    admin_member_id,
        admin_id,admin_name,
        email,
    ...
    console.log("관리자 테이블 전테 로우건수 조회: ", adminCount);
...
router.post("/create", async (req, res, next) => {
    ...
    //hash('사용자가 입력한 암호',암호화횟수)***암호화횟수가 높을수록 보안이 강화됨
    const encrypedPassword = await bcrypt.hash(admin_password, 12);
    ...
    const admin = {
        company_code,
        admin_id,
        admin_password: encrypedPassword,
        admin_name,
        email,
        telephone,
        dept_name,
