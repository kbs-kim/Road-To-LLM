...
function fnPlus(a,b){
    ...
    //콘솔로깅 전용함수를 호출해서 콘솔에 계산 결과값을 출력한다.
    fnLogging(c); // -> 추가한 코드 : 함수에서 특정함수를 호출한다.
    return c;
}

//특정함수를 매개변수로 전달해서 해당 함수내 특정 위치에서 콜백함수가 실행되게 구현한다.
//콜백함수 개념 이해하기****
// 콜백함수를 이용하면 특정함수내에서 순차적 프로그래밍이 가능하다.
...
// 이전함수의 계산된 결과값에 기본 배송비 3000원을 추가한 로직으로 콘솔출력
...
//fnPlusCallBack()함수내에서 fnLogging()함수를 호출하여 순차적인 (동기방식)프로그래밍 가능.
...
//자바스크립트 함수는 특정함수에 파라메터(매개변수)로 전달 가능합니다.
const result1 = fnPlusCallBack(1000,2000,fnLogging);


//배송비가 추가되는 로직이 돌아가는 fnLoggingDelivery()함수를 콜백함수로 전달한다.
const result2=fnPlusCallBack(1000,2000,fnLoggingDelivery);


//콜백함수를 구현(사용)하는 이유는 기본함수가 실행될때 기본 함수내에서 
//전달된 콜백함수를 순차적(동기방식)으로 실행될수 있게 강제하기 위한 수단으로 사용된다.
