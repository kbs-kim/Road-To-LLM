...
var db= require('../models/index.js'); //models의 index.js파일에서 db객체를 참조함 %%%
...
router.get('/list', async(req,res)=>{
    ...
    res.render('article/list.ejs',{articles:articles}); //게시글 목록정보를 list.ejs에 전달함 %%%
...
router.post('/create',async(req,res)=>{
    ...
    const display_code = req.body.display_code; //게시글 노출여부

    //Step2: article 테이블에 등록할 json데이터 생성하기
    //주의 / 중요: 반드시 json데이터 속성명은 modles/article.js 모델파일의 속성명과 일치해야함 ^^^^
    const article = {
        board_type_code:1,
        title:title,
        article_type_code:0,
    ...
    //create()메소드는 ORM Framework에 의해 INSERT INTO artuces VALUES() 쿼리문으로 자동변환되어
    //DB서버에 전소오디어 DB서버에서 실행되고 실제 저장된 단일 게시글 DATA를 DB서버에서 반환한다.
...
router.post('/modify', async(req,res)=>{
    ...
    //Step2: 수정된 게시글 정보를 json데이터로 생성한다.
...
router.get('/delete/:id', async(req,res)=>{
    ...
    //Step1: 현재게시글 고유번호를 추출한다.
    const articleIdx = req.params.id;

    //Step2: 해당 게시글 번호를 기준으로 단일 게시글 정보를 조회한다.
    //SELECT * FROM articles WHERE article_id =1; SQL구문이 백엔드에서 만들어져서
    //DB서버로 전송되어 실행되고 그 결과를 백엔드에서 반환받아서 변수에 저장한다.
    ...
    //DB에서 해당 게시글 번호와 일치하는 단일 게시글 정보조회
    //기존 게시글 db삭제처리후
    res.redirect('/article/list'); //목록 페이지로 이동
