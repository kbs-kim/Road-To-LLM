...
// 웹페이지 지식베이스 Agent 사용하기
// 웹페이지 크롤링을 위한 npm i cheerio 설치하기
...
export default async function handler(
  ...
  try {
    ...
    if (req.method == "POST") {
      ...
      //Step3:웹페이지 크롤링을 위한 CheerioWebBaseLoader 객체 생성하기
      const loader = new CheerioWebBaseLoader("https://kbs-kim.github.io/frontend/typescript-type-annotation/");

      //웹페이지 내용 로딩하기
      const rawDocs = await loader.load();
      console.log("Cheerio를 통해 로딩한 로우 웹페이지 데이터:",rawDocs);

      //Step4:텍스트 스플릿터 객체 생성하기
      ...
      //MemorVectorStore.fromDocuments(임베딩된문서, 사용할 임베딩모델 처리기);
      const vectorStore = await MemoryVectorStore.fromDocuments(docs,new OpenAIEmbeddings());

      //Step6: 메모리 베거 저장소에서 사용자 질문으로 Query질문하기
